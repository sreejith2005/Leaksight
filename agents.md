...
-   Auth: `admin@test.com` / `PZAD-QyiIWCBct2iRxvEkQ`
...
```
.venvScriptspython.exe -m uvicorn backend.app.main:app --reload --host 0.0.0.0 --port 8000
```
...
-   Apply with: `& "C:UsersLENOVODownloadsLeaksight v1 -1.venvScriptspython.exe" -m alembic upgrade head`
...
-   `camelot-py[cv]` — complex table extraction from PDFs
...
```
.venvScriptspython.exe -m pytest backend/tests/tools/ -v --tb=short
```
...
```
.venvScriptspython.exe _upload_demo_data.py
```
...
```
.venvScriptspython.exe _generate_tool_a_demo_data.py   # creates Excel files.venvScriptspython.exe _run_tool_a_demo.py              # full end-to-end verification
```
